...
impl DateParsing for DateParser {
    ...
    fn parse_synonym(&self, synonym: &str) -> Result<DateTime<Utc>, DateError> {
        ...
        let date = match synonym_lower.as_str() {
            ...
            // Weekdays
            "monday" | "tue" | "tuesday" | "wed" | "wednesday" | "thu" | "thursday" | "fri"
            ...
            _ => {
                return Err(DateError::UnknownSynonym {
    ...
    fn calculate_relative_date(
        ...
        let result = match unit {
            ...
            "y" | "year" | "years" => {
                ...
                date = date
                    .with_year(new_year)
                    .ok_or_else(|| DateError::InvalidRelative {
                        expression: expression.to_string(),
                    })?;
                self.timezone
                    .from_local_datetime(&date.and_hms_opt(0, 0, 0).unwrap())
...
impl DateParser {
    ...
    fn start_of_month(&self, date: DateTime<Utc>) -> Result<DateTime<Utc>, DateError> {
        let first_day = NaiveDate::from_ymd_opt(date.year(), date.month(), 1).ok_or_else(|| {
    ...
    fn start_of_year(&self, date: DateTime<Utc>) -> Result<DateTime<Utc>, DateError> {
        let first_day =
            NaiveDate::from_ymd_opt(date.year(), 1, 1).ok_or_else(|| DateError::InvalidFormat {
    ...
    fn end_of_year(&self, date: DateTime<Utc>) -> Result<DateTime<Utc>, DateError> {
        let last_day = NaiveDate::from_ymd_opt(date.year(), 12, 31).ok_or_else(|| {
            DateError::InvalidFormat {
    ...
    fn start_of_quarter(
        ...
        let first_day = NaiveDate::from_ymd_opt(date.year(), month, 1).ok_or_else(|| {
            DateError::InvalidFormat {
    ...
    fn add_month(&self, date: NaiveDate) -> NaiveDate {
        if date.month() == 12 {
            ...
            NaiveDate::from_ymd_opt(date.year(), date.month() + 1, date.day()).unwrap_or_else(
                || {
                    // Handle month-end edge cases
                    let mut day = date.day();
                    while day > 28 {
                        if let Some(new_date) =
                            NaiveDate::from_ymd_opt(date.year(), date.month() + 1, day)
                        {
                            return new_date;
                        }
                        day -= 1;
