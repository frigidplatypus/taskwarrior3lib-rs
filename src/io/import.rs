...
impl DefaultTaskImporter {
    ...
    pub fn import_json<R: Read>(
        ...
        let tasks: Vec<Task> = serde_json::from_reader(reader).map_err(TaskError::Serialization)?;
    ...
    fn parse_csv_line(
        ...
        if values.len() != headers.len() {
            return Err(TaskError::InvalidData {
                message: format!(
                    "CSV line has {} values but {} headers",
                    values.len(),
                    headers.len()
                ),
            });
...
pub fn import_tasks_from_file(
    ...
    let file = std::fs::File::open(file_path).map_err(TaskError::Io)?;
...
mod tests {
    ...
    fn test_format_detection() {
        ...
        assert_eq!(
            importer.detect_format(&mut tw_cursor).unwrap(),
            ImportFormat::TaskwarriorLegacy
        );
    }
}
