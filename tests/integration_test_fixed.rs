...
use taskwarriorlib::{
    task::{TaskStatus, Priority, manager::{DefaultTaskManager, TaskManager, TaskUpdate}},
    reports::ReportManager,
    query::{TaskQueryBuilderImpl, TaskQueryBuilder},
...
fn create_test_manager(temp_dir: &TempDir) -> Result<DefaultTaskManager, Box<dyn std::error::Error>> {
...
fn test_comprehensive_workflow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tasks_data = vec![
        ...
        ("Update documentation", "Documentation", Some(Priority::Medium)),
